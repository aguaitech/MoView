...
import type { Config, FaceResult, BodyResult } from '@vladmandic/human';
...
const detectorConfig: Partial<Config> = {
  cacheSensitivity: 0,
...
function estimateMovement(previous: Uint8ClampedArray | null, current: Uint8ClampedArray): { score: number; data: Uint8ClampedArray } {
  if (!previous) {
...
function embeddingToArray(input: unknown): number[] {
  if (!input) {
    return [];
  }
...
export class PresenceDetector {
  private human = new Human(detectorConfig);
  ...
  async detect(video: HTMLVideoElement, settings: AppSettings): Promise<DetectionResult> {
    if (!this.initialized) {
      await this.init();
    }

    const result = await this.human.detect(video);
    const faces = ((result.face ?? []) as FaceResult[]).filter((face) => (face.score ?? 0) > 0.3);
    const bodies = (result.body ?? []) as BodyResult[];
    ...
    const faceOrBodySuggestVisitor =
      faceOrBodyTrigger && (hasUnknownFaces || extraBodiesPresent || !matchedSafe.recognized);
  ...
  async captureSafeFace(video: HTMLVideoElement, label: string): Promise<SafeFaceProfile> {
    if (!this.initialized) {
      await this.init();
    }
  ...
  private calculateMovement(video: HTMLVideoElement, settings: AppSettings): number {
    if (!this.ctx) {
      return 0;
    }
  ...
  private matchSafeFaces(
    faces: FaceResult[],
    safeFaces: SafeFaceProfile[],
    threshold: number
  ): { recognized: boolean; matchedProfileIds: string[]; matchedCount: number; onlyRecognizedSafe: boolean } {
    ...
    for (const face of faces) {
      ...
      let bestMatch: SafeFaceProfile | null = null;
      let bestScore = -1;
